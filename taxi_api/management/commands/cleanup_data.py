...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if keep_records > 0:
            ...
            trips_to_delete = trips_to_delete.order_by(
                '-pickup_datetime'
            )[keep_records:]
        ...
        if older_than_days:
            ...
            trips_to_delete = trips_to_delete.filter(
                pickup_datetime__lt=cutoff_date
            )
            self.stdout.write(
                f'Will delete records older than {older_than_days} days'
            )
        ...
        if not dry_run:
            ...
            if records_to_delete > 0:
                confirm = input(
                    f'Delete {records_to_delete:,} records? (y/N): '
                )
                if confirm.lower() != 'y':
        ...
        self.stdout.write(
            f'  Records deleted: {initial_trips - final_trips:,}'
        )
