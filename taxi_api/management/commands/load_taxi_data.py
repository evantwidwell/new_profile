...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            ...
            type=int,
            default=1,
            help='Month of data to load (default: 1)'
        )
        parser.add_argument(
            '--clear-data',
            action='store_true',
    ...
    def load_taxi_zones(self):
        ...
        zones_url = ("https://d37ci6vzurychx.cloudfront.net/misc/"
        ...
        try:
            ...
            self.stdout.write(
                self.style.ERROR(f'Error loading taxi zones: {str(e)}')
            )
    ...
    def load_trip_data(self, year, month, sample_size, get_all, batch_size):
        ...
        try:
            ...
            self.stdout.write(
                self.style.SUCCESS('Successfully loaded taxi data')
            )
            
        except Exception as e:
            self.stdout.write(
                self.style.ERROR(f'Error loading trip data: {str(e)}')
            )
    ...
    def process_all_records(self, conn, temp_file, batch_size, total_records):
        ...
        for batch_idx in range(num_batches):
            ...
            chunk_data = self.get_cleaned_chunk(
                conn, temp_file, start_idx, batch_size
            )
            ...
            self.stdout.write(
                f'Progress: {progress:.1f}% ({end_idx:,}/{total_records:,} '
                f'records)'
            )
        
        self.stdout.write(
            self.style.SUCCESS(f'Loaded {total_loaded:,} valid trips')
        )
    ...
    def get_cleaned_chunk(self, conn, temp_file, offset, limit):
        ...
        try:
            ...
            self.stdout.write(
                self.style.WARNING(f'Error reading chunk: {str(e)}')
            )
            return []
    ...
    def get_cleaned_sample(self, conn, temp_file, sample_size):
        ...
        try:
            ...
            self.stdout.write(
                self.style.WARNING(f'Error reading sample: {str(e)}')
            )
            return []
